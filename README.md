...
*   [Scripts](#scripts)
    ...
    *   [capture\_and\_detect](#capture_and_detect):
        A script which provides a [CaptureAndDetect](#captureanddetect)
        service.
    *   [monitor\_and\_detect](#monitor_and_detect)
...
+   A trained model and a label file.
    ...
    can be converted for this module by the script
    [prepare.py](scripts/prepare.py).

## <a name="preparation">Preparation</a>
To use this module, a model file and a label file are required.
...
and converts them so that they can be used with this module.
...
1.  Build this package.  
    ...
    This module includes only Python scripts which do not require compilation,
...
1.  Start a node which provides a service.  
    ```
...
+   INPUT
    ...
    from.  
    `capture_timeout` means a timeout. If the recognition does not finish
...
+   [capture\_and\_detect\_client.py](scripts/capture_and_detect_client.py)  
    This script calls a [CaptureAndDetect](#captureanddetect) service with
    a request for *each image given as arguments* and then display returned
    information.
